...
- **Status**: `open`
- **Remediation**: The root contributing cause was that the literal format of the "Council session reference" required by `CLAUDE.md` was never defined, making omission easy. It is now defined: every L2 commit/merge message ends with a `Council-Authorized: ORPI-<YYYY-MM-DD>` trailer (this session: `ORPI-2026-06-07`), first demonstrated by commit `e4103b1` which records this finding. Forward fixes: (a) a project-level `SessionStart` hook (`.claude/settings.json`, merged in PR #3, commit `fa2879a`) loads `CLAUDE.md` into context when a session starts inside the repository — this *mitigates but does not fully achieve* "regardless of working directory", because project-level settings only activate when the session's working directory is the repo; the original PR #1 failure (cwd `~`, outside the repo) would not be caught by a project-level hook, so full coverage additionally requires a machine-specific user-level hook in `~/.claude/settings.json` per Operator, or the operational discipline of always starting sessions with the repo as the working directory; (b) Council pre-merge review explicitly verifies the message trailer — its absence in PR #1 was also a Council review miss.
